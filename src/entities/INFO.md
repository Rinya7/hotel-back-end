...
├── Room.ts               # Кімнати
├── Stay.ts               # Проживання
├── GuestAccessToken.ts   # Токени доступу гостя
├── StayGuest.ts          # Детальна інформація про гостей
├── StayStatusLog.ts      # Логи змін статусів Stay
├── RoomStatusLog.ts      # Логи змін статусів Room
└── RoomCleaningLog.ts    # Логи прибирання кімнат
```
...
- `guestTokens` — зв'язок з токенами доступу гостя
...
**Поля:**
- `id` — унікальний ідентифікатор
- `stay` — зв'язок з проживанням (Stay)
- `token` — секретний токен для доступу (унікальний, 64 hex символи)
- `createdAt` / `updatedAt` — автоматичні timestamp'и
- `expiresAt` — дата прострочення (null = безстроково)
- `revokedAt` — дата відкликання доступу (null = активний)
- `lastUsedAt` — дата останнього використання (для статистики)
...
### Stay ↔ GuestAccessToken
- **Один до багатьох** — одне проживання може мати кілька токенів доступу
- **Зовнішній ключ:** `guest_access_token.stayId → stay.id`
- **Каскадне видалення:** при видаленні Stay видаляються всі токени
