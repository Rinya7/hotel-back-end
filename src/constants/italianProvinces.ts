// src/constants/italianProvinces.ts
// Список провинций/регионов Италии (отсортирован по алфавиту, только итальянские названия)

export interface ItalianProvince {
  code: string; // Код провинции (обычно 2 буквы)
  name: string; // Название провинции на итальянском
  region: string; // Регион на итальянском
}
...
].sort((a, b) => a.name.localeCompare(b.name, "it")); // Сортировка по итальянскому алфавиту

/**
 * Получить провинцию по коду
 */
export function getProvinceByCode(code: string): ItalianProvince | undefined {
  return ITALIAN_PROVINCES.find((p) => p.code === code);
}

/**
 * Получить провинции по региону
 */
export function getProvincesByRegion(region: string): ItalianProvince[] {
  return ITALIAN_PROVINCES.filter((p) => p.region === region).sort((a, b) =>
